//
//  MarketplaceBrowserViewV2.swift
//  claude-maestro
//
...
struct MarketplaceBrowserViewV2: View {
    ...
    @State private var showUserCreator: Bool = false
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showContributeSheet) {
            ContributePluginSheet(onDismiss: { showContributeSheet = false })
        }
        .sheet(isPresented: $showUserCreator) {
...
// MARK: - Contribute Plugin Sheet

struct ContributePluginSheet: View {
    let onDismiss: () -> Void

    @State private var contributionType: ContributionType = .skill
    ...
    enum ContributionType: String, CaseIterable {
        ...
        var description: String {
            switch self {
            case .skill: return "Enhance Claude's capabilities with specialized knowledge"
            case .command: return "Add slash commands for common workflows"
            case .mcp: return "Connect Claude to external services and APIs"
            case .hook: return "Automate actions with event-driven hooks"
            }
        }
    }
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            VStack(spacing: 12) {
                ...
                Text("Contribute to Marketplace")
                    .font(.system(size: 18, weight: .bold, design: .rounded))

                Text("Share your plugins with the community")
                    .font(.system(size: 12))
                    .foregroundColor(.secondary)
            }
            .padding(24)
                ...
                LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 10) {
                    ForEach(ContributionType.allCases, id: \.self) { type in
                        ContributionTypeCard(
                            type: type,
                            isSelected: contributionType == type,
                            onSelect: { contributionType = type }
                        )
                    }
                }
            }
            ...
            VStack(alignment: .leading, spacing: 10) {
                HStack(spacing: 8) {
                    Image(systemName: contributionType.icon)
                        .foregroundColor(contributionType.color)
                    Text(contributionType.rawValue)
                        .font(.system(size: 13, weight: .semibold))
                }

                Text(contributionType.description)
                    .font(.system(size: 12))
                    .foregroundColor(.secondary)
                ...
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    Text("1. Create a new directory in ~/.claude/\(contributionType == .skill ? "skills" : contributionType == .command ? "commands" : "plugins")/")
                        .font(.system(size: 11, design: .monospaced))

                    Text("2. Add your \(contributionType == .skill ? "SKILL.md" : contributionType == .command ? "command.md" : ".mcp.json") file")
                        .font(.system(size: 11, design: .monospaced))

                    Text("3. Submit a PR to your marketplace repository")
                        .font(.system(size: 11, design: .monospaced))
                }
                .foregroundColor(.secondary)
                .padding(12)
            ...
            HStack(spacing: 12) {
                ...
                Link(destination: URL(string: "https://github.com/anthropics/claude-plugins-official")!) {
...
// MARK: - Contribution Type Card

struct ContributionTypeCard: View {
    let type: ContributePluginSheet.ContributionType
    let isSelected: Bool
    let onSelect: () -> Void
            ...
            .background(
                RoundedRectangle(cornerRadius: 12)
                    .fill(
                        isSelected
                            ? type.color
                            : (isHovered ? type.color.opacity(0.1) : Color.primary.opacity(0.03))
                    )
            )
            .overlay(
                RoundedRectangle(cornerRadius: 12)
                    .strokeBorder(
                        isSelected ? Color.clear : type.color.opacity(isHovered ? 0.3 : 0.1),
                        lineWidth: 1
                    )
            )
        }
